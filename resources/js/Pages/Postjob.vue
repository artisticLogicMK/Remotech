...
const form = useForm({
    ...
    description:     null,
    type:            null,
    skills:          computed(() => skills.value.join(' '))  ?? null,
    qualifications:  null,
    salary:          salary,
...
<template>
    ...
    <div class="grow bg-gray-50 px-3 md:px-6 overflow-auto">
        ...
        <form @submit.prevent="form.post(route('storeJob'))">

            <div class="flex items-center justify-between border-b border-gray-200 py-3">
                <div class="text-gray-500">
                    <div class="text-lg">Create a Job Listing.</div>
            ...
            <div class="max-w-4xl">
                ...
                <div class="flex justify-between items-center mt-2 mb-8">
                    <p v-if="Object.keys(errors).length" class="text-red-500/80 text-sm font-semibold mt-2">Some errors found...</p>
                    <span v-else></span>

                    <button type="submit"
                        class="bg-sky-400 text-white text-sm font-semibold rounded px-3 py-1.5 hover:bg-sky-500"
